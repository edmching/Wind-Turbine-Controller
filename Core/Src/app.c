...
void ReadSensor_Task(SensorVar_t* sensorVars)
{
  ...
  sensorVars->current[1]           = ((ADC_current*ADC_VAL_TO_VOLTS - VCC/2.0)/0.4)*SENSOR_RESOLUTION;
		  	  	  	  	  	  	  	 /*SENSOR_RESOLUTION*map_fvalues(
                                                        ADC_current,
                                                        I_SENS_ADC_ZERO_VAL,
                                                        I_SENS_ADC_3V3_VAL,
                                                        I_SENS_MIN_AMPS,
...
void StepMotor_Task(SensorVar_t* sensorVars)
{
  ...
  ///On power cycle, initialize the current positon as the home position
  if(on_power_cycle == true)
  {
...
void PrintVariables(SensorVar_t* sensorVars)
{
  ...
  printf(
	"\r\n  power = %d.%06dW, adc_val[0] = %d, V_rectifier = %d.%03dV, adc_val[1] = %d, current = %d.%03dA, adc_val[3] = %d, V_load = %d.%03dV",
    power_integral, power_decimal,
    g_adc_val[0],
    voltage_integral,voltage_decimal,
    g_adc_val[1],
